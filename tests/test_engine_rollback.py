...
def test_arm_calls_arm_rollback_with_int_prefix():
    ...
    _OPS.arm_rollback([a, b, "not_armable"], prefix_len=42)
...
def test_full_acceptance_skips_rollback_replay():
    c = _ArmableCache()
    replay_ns = _OPS.restore_after_acceptance(
        [c],
        target_len=100,
        acceptance_length=4,
        drafted_tokens=4,
...
def test_cleanup_clears_target_transients_and_empties_lists():
    ...
    target = [c]
    draft = ["draft_entry"]
    _OPS.cleanup_generation_caches(target, draft)
    assert target == []
    assert draft == []
    assert c._tape is None
